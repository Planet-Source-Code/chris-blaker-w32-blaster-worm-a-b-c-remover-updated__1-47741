...
Begin VB.Form frmMain 
   ...
   Caption         =   "W32.BLASTER.WORM - Remover"
   ClientHeight    =   3262
   ClientLeft      =   42
   ClientTop       =   364
   ClientWidth     =   5474
   ...
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "w32.blaster.worm remover for Windows."
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   8
         Charset         =   0
...
Private Sub cmdBegin_Click()
    ...
    CreateNewKey "", hKey
    SetKeyValue HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion\Run", "decom remover", Addtoreg, REG_SZ  ' Add the program to the registry so it will perform it's second task after reboot
    DeleteValue HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion\Run", "windows auto update"  ' Remove the virus from the registry.
    DeleteValue HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion\Run", "Microsoft Inet Xp.."  ' Remove the virus from the registry.
...
If cmdBegin.Tag = "1" Then  ' If the application has already done the first part, proceed to the next.
    Me.Hide
    Kill "C:\WINDOWS\SYSTEM32\teekids.exe"  '----|
    Kill "C:\WINDOWS\SYSTEM32\penis32.exe"  '----|
    Kill "C:\WINDOWS\SYSTEM32\msblast.exe"  '----| Remove the virus from it's folders.
    Kill "C:\WINDOWS\SYSTEM32\root32.exe"   '----|
    Kill "C:\WINDOWS\SYSTEM32\index.exe"    '----|
    
    Kill "C:\WINNT\SYSTEM32\teekids.exe"    '----|
    Kill "C:\WINNT\SYSTEM32\penis32.exe"    '----|
    Kill "C:\WINNT\SYSTEM32\msblast.exe"    '----| Remove the virus from it's folders.
    Kill "C:\WINNT\SYSTEM32\root32.exe"     '----|
    Kill "C:\WINNT\SYSTEM32\index.exe"      '----|
    
    
    DeleteValue HKEY_LOCAL_MACHINE, "Software\Microsoft\Windows\CurrentVersion\Run", "decom remover"
